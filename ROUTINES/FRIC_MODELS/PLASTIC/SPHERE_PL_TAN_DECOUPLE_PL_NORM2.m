function [fxyn, dfxynduxyn, rq0, fx0, fy0, deltam, Fm, am, ...
    dfduxyn0, dfdfxy0, dfddeltam, a, dadun] = SPHERE_PL_TAN_DECOUPLE_PL_NORM2(pars, uxyn, uxyn0, rq0, fx0, fy0, rq, wq, deltam, Fm, am, varargin)
...
%
% NOTES:
%   1. Since the derivatives returned from SPHERE_PLASTIC_PRE2 include all
...
%   one term.
%   2. The method of re-establishing contact is out of date and does not
%   consider the fact that the asperity is now at a lower position - UPDATE
%   THIS.
%   3. The derivatives of tangential information w.r.t. normal information
...
%   SPHERE_PLASTIC_PRE2 for dfndun, so it is very difficult to verify these
%   derivatives. 
    ...
    if(uxyn0(1, 3) < 0)
        ...
        % The following works for linearly estimating the tangential 
        % displacement the normal displacement crosses zero. However, with
        % plasticity 0 =/= re-established contact, so this is wrong
        % Given the complexity of harmonic derivatives and the high number
        % of samples per cycle compared to QSMA, it is removed here. 
        % Therefore, it is assumed that contact is re-established at the
        % previous instant. 
...
% %         
% %         duxyn0duxyn0(1, 1) = (0 - uxyn0(3))./(uxyn(3) - uxyn0(3)).*(-1) + 1;
% %         duxyn0duxyn0(2, 2) = (0 - uxyn0(3))./(uxyn(3) - uxyn0(3)).*(-1) + 1;
% %         duxyn0duxyn0(1:2, 3) = (-uxyn(3)./(uxyn(3) - uxyn0(3)).^2).*(uxyn(1:2) - uxyn0(1:2));
% %         
% %         
% %         %now update uxyn0
...
% % %         uxyn0(1, 3) = 0; %Should not matter.
    ...
    dSysdun, daddeltam, dSysddeltam, deltabar, ddeltabar_ddeltam] ...
        = SPHERE_PLASTIC_PRE2(pars, uxyn, uxyn0, rq0, fx0, fy0, rq, wq, deltam, Fm, am, dFmddeltam, damddeltam);

    
%     % Basic Plasticity Model 1:
%     % Plasticity only considered based on avg. shear stress.
%     fs = pars.Sys/sqrt(3)*(pi*a^2);
%     dfsdun = pars.Sys/sqrt(3)*(2*pi*a*dadun);
    
    
%     % Basic Plasticity Model 2:
%     % Consider average normal pressure and average shear stress in the
%     % limit of yielding for f_s
%     tn = fxyn(3)/(pi*a^2);
%     dtndun = -2*fxyn(3)/(pi*a^3)*dadun+dfxynduxyn(3,3)/(pi*a^2);
%     
%     % Constant yield stress
%     ts = sqrt((pars.Sys^2 - tn^2)/3);
%     dtsdun = 1/(2*ts)*(-2*tn/3)*dtndun;
    
    % Basic Plasticity Model 3:
    % Consider average normal pressure and average shear stress in the
    ...
    %% CEB Limit for force
    % This comes from:
    %       Eriten et al., 2010, Physics-based modeling for partial slipi
    ...
    % Updated from Johnson's book on 11/9/2020
    kt = 8.*pars.Gstar*a;
